...
public class GeosupportClient {
    ...
    public void callFunctionBL() {
        ...
        doCall(Function.FBL, buildParams("geosupportFunctionCode", Function.FBL, "bblBoroughCodeIn", boroCode,
            "bblTaxBlockIn", block, "bblTaxLotIn", lot), true, this.displayEmptyFieldValues);
    }

    public void callFunctionBN() {
        showFunctionHeader(Function.FBN);
        String bin = promptAndReadValue("BIN");
        doCall(Function.FBN, buildParams("geosupportFunctionCode", Function.FBN, "buildingIdentificationNumberIn", bin),
            true, this.displayEmptyFieldValues);
    }
    ...
    public void callFunction2() {
        ...
        doCall(Function.F2,
            buildParams("geosupportFunctionCode", Function.F2, "streetName1In", crossStreet1, "streetName2In",
                crossStreet2, "compassDirection", compassDirection, "boroughCode1In", boroCode),
    ...
    public void callFunction3() {
        ...
        String boroCode = promptAndReadValue("borough code");
        doCall(Function.F3,
            buildParams("geosupportFunctionCode", Function.F3, "streetName1In", onStreet, "streetName2In", crossStreet1,
                "streetName3In", crossStreet2, "compassDirection", compassDirection, "boroughCode1In", boroCode),
            true, this.displayEmptyFieldValues);
    }
    ...
    public void callFunctionDG() {
        ...
        doCall(Function.FDG, buildParams("geosupportFunctionCode", Function.FDG, "boroughCode1In", boroCode,
            "streetCode1In", streetCode), false, this.displayEmptyFieldValues);
    }

    public void callFunctionHR() {
        showFunctionHeader(Function.FHR);
        doCall(Function.FHR, buildParams("geosupportFunctionCode", Function.FHR), false, this.displayEmptyFieldValues);
    }

    public String chooseFunction() {
        System.out.println();
        System.out.println();
        System.out.print(String.format("Enter a function(%s), '%s' to configure display or '%s' to quit: ",
            FUNCTION_MENU.toString(), CONFIG_CHOICE, QUIT_CHOICE));
    ...
    private void doCall(String functionName, Map<String, Object> parameters, boolean doExtended,
            boolean displayEmptyFields) {
    ...
    private void doCallFunction1(String functionName, boolean doExtended) {
        ...
        if (isB7SC(street)) {
            doCall(functionName,
                buildParams("geosupportFunctionCode", functionName, "houseNumberIn", houseNumber, "streetCode1In",
                    street, "boroughCode1In", boroCode, "zipCodeIn", zipCode),
                doExtended, this.displayEmptyFieldValues);
        }
        else {

            doCall(functionName,
                buildParams("geosupportFunctionCode", functionName, "houseNumberIn", houseNumber, "streetName1In",
                    street, "boroughCode1In", boroCode, "zipCodeIn", zipCode),
                doExtended, this.displayEmptyFieldValues);
        }
    }
    ...
    private void showCombineWorkAreasConfiguration() {
        System.out.println(
            String.format("'Combine work areas in results?'='%s'", String.valueOf(this.combineWorkAreaResults)));
    }
