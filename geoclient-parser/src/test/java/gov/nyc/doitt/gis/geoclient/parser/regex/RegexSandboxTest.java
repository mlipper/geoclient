...
public class RegexSandboxTest {
    ...
    private static final Pattern ENDS_WITH_USA = Pattern.compile(USA.pattern() + "\\s*$", Pattern.CASE_INSENSITIVE);
    private static final Pattern ENDS_WITH_ZIP = Pattern.compile("((?:\\d{5})(?:-\\d{4})?)(?:(\\s|[,])+USA)?\\s*$",
        Pattern.CASE_INSENSITIVE);

    private static final String REGEX = "(?:F/O|IFO|I/F/O|FO)?\\b\\s*" + "(\\d+(?:[A-Z]{1}\\b)?)"
            + "(\\s?[A-Z]-[A-Z]\\b)?" + "(\\s?-{1,2}\\s?\\w+(?:/\\d+)?)?" + "((?:\\s?\\d+/\\d+(?:-[A-Z])?){0,2})"
            + "(\\s?\\d+/\\d+)?"
            + "(\\s?\\d*\\s?(?:REAR|INTER|GARAGE|GAR|GARAG|FLOOR|FL|AIRRGTS|FRTA|BB|AB|BA|AA|ABCD|RSTU)\\b(?:-[A-Z])?)?"
            + "(\\s?\\d*\\s?[A-Z&&[^NSEW]]{1}\\b(?!'))?" + "(\\s?\\d+[A-Z]{1}\\b)?"
            + "(\\s?[A-Z&&[^NSEW]]{1}\\d+\\b(?:/\\d)?)?" + "(\\s?&?\\d+\\s+(?=\\d+))?" + "(#\\d)?" + "(.*)";
    private static final Pattern STARTS_WITH_HOUSE_NUMBER = Pattern.compile(REGEX, Pattern.CASE_INSENSITIVE);

    private String[] usa = new String[] { "USA should not a match", "USA America US", "", "xxxUSA", "xxx,USA",
            "xxx,USA ", "xxx USA", "xxx USA ", "xxx , USA  ", "This is the United States of America", "xxx America  ",
            " US" };
    ...
    private String[] houseNumbers = new String[] { "280 RSD", "519 Front East 12th Street, Manhattan",
            "625 Rear Smith Street, Brooklyn", "120 1/2 First Avenue, Manhattan", "240-55 1/3 Depew Avenue, Queens",
            "469 1/4 Father Capodanno Blvd.", "470 A West 43rd Street, Manhattan", "171C Auburn Avenue, Staten Island",
