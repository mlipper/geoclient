...
public class CompassDirectionTaskBuilder extends TaskBuilderSupport implements SpawnedSearchTaskBuilder {
    public CompassDirectionTaskBuilder(CountyResolver countyResolver, GeosupportInvoker geosupport,
        ...
        for (Search search : searchResult.inputForSubSearches()) {
            if (search.getResponse().isCompassDirectionRequired()) {
                tasks.add(new IntersectionSearchTask(newIntersectionRequest((IntersectionRequest) search.getRequest(),
                    InputParam.COMPASS_DIR_NORTH_VALUE), geosupport, statusReader));
                tasks.add(new IntersectionSearchTask(newIntersectionRequest((IntersectionRequest) search.getRequest(),
                    InputParam.COMPASS_DIR_SOUTH_VALUE), geosupport, statusReader));
                tasks.add(new IntersectionSearchTask(newIntersectionRequest((IntersectionRequest) search.getRequest(),
                    InputParam.COMPASS_DIR_EAST_VALUE), geosupport, statusReader));
                tasks.add(new IntersectionSearchTask(newIntersectionRequest((IntersectionRequest) search.getRequest(),
                    InputParam.COMPASS_DIR_WEST_VALUE), geosupport, statusReader));
            }
        }
        return tasks;
    }
