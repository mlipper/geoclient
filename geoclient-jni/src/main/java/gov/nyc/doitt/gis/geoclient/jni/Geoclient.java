...
/**
 * Defines Geoclient's public Java API for interacting with <code>JNI</code> and
 * native <code>C</code> code linked to Geosupport native libraries.
 * 
 * The code in this class was originally generated by the <code>gluegen</code>
 * framework but that proved unnecessary when using Geosupport's work area
 * <code>C/C++</code> API. This is because the overall work area sizes remain
 * constant and field changes by DCP can be adjusted using positional mappings
 * (i.e., <code>geoclient-core</code>'s <code>geoclient.xml</code> file).
 * 
 * @author mlipper
 * @since 1.0
 */
