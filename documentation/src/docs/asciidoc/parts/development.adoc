...
At this time, the New York City Department of City Planning only distributes binaries for Linux and Windows compiled for the 64-bit Intel/AMD instruction set (`x86_64`/`AMD64`). `ARM64` and the macOS platform (even on Intel CPUs) are *not* supported.
...
* 64-bit Geosupport
* 64-bit JDK 21+ ("Full" JDK required. JRE by itself will *not* build)
* Gradle 9.3+
...
[,sh]
----
 docker build -t geoclient:latest --platform linux/amd64 -f images/build.Dockerfile .
----
...
[,sh]
----
 docker run --rm -t -p 8080:8080 geoclient:latest
----
...
[,sh]
----
 docker build -t geoclient:latest --platform linux/amd64 -f images/run.Dockerfile .
----
+
...
[,sh]
----
 docker build --build-arg JARFILE=/path/to/geoclient-service.jar -t geoclient:latest -f images/run.Dockerfile .
----
...
[,sh]
----
 docker run --rm -t --mount source=geosupport-latest,target=/opt/geosupport -p 8080:8080 geoclient:latest
----
...
* The `geoclient:latest` image.
* The `geosupport-latest` volume.

Note that the `GEOCLIENT_IMAGE` and `GEOSUPPORT_VOLUME` environment variables are defaulted to `geoclient:latest` and `geosupport-latest`, respectively by the `images/.env` file.
